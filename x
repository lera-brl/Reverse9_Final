...
ui_texts = {
    ...
    "visible": {
        ...
        "x": 56, "y": 670, "size": SMALL_UI_SIZE, "color": WHITE
    },
    "glasses": {
        "text": "No hair or glasses.",
        "x": 56, "y": 710, "size": SMALL_UI_SIZE, "color": WHITE
    },
...
def stage_3_capture_and_flash_live_mask(frame, clean_frame, results):
    ...
    for alpha in np.linspace(0, 1, 10):
        ...
        cv2.waitKey(1)  # минимальная задержка для обновления

    # — Запускаем backend: эмоции + FaceSwap —
    generated_image, category, dominant = run_backend_pipeline(clean_frame)
...
def run_backend_pipeline(clean_frame):
    ...
    if source_img is not None and target_img is not None:
        ...
        if len(source_faces) > 0 and len(target_faces) > 0:
            ...
            print("n- Emotion Analysis -")
            for emotion, value in emotions.items():
                print(f"{emotion.capitalize()} : {int(value)}%")
                print(f"\nDoninant Emotion: {dominant}")
...
def stage_4_loading(frame, clean_frame, results):
    ...
    if backend_started_time is None:
        backend_started_time = time.time()
        generated_image, category, dominant = run_backend_pipeline(clean_frame) # <<< запуск backend как только начинается stage 4
        final_generated_image = generated_image

    # Заголовок
    loading_text = "You are being categorised, transformed and fragmented."
    cv2.putText(frame, loading_text, (60, 300), FONT, HEADING_SIZE, BLACK, 2)

    # Анимированные точки загрузки
    elapsed = time.time() - backend_started_time
    dot_count = int(elapsed * 1.5) % 4
    loading_dots = "." * dot_count
    cv2.putText(frame, f"New identity is loading{loading_dots}", (60, 360), FONT, SUBHEADING_SIZE, BLACK, 2)

    # Италики — псевдо-курсив путём сдвига текста
    cv2.putText(frame, "It will only take a moment. You won’t even notice.",
                (60, 420), FONT, SMALL_UI_SIZE, BLACK, 1)
    cv2.putText(frame, "Algorithms appreciate your engagement.",
                (60, 460), FONT, SMALL_UI_SIZE, BLACK, 1)

    if elapsed >= 9:
        current_stage = 5  # Переход к Rebirth
...
def stage_5_rebirth(frame):
    ...
    elif time.time() - stage_5_rebirth.start_time >= 49:
        current_stage = 6
...
def stage_6_goodbye(frame):
    ...
    elif time.time() - stage_6_goodbye.start_time >= 5:
...
while cap.isOpened():
    ...
    # === Показываем live-видео только если это НЕ стадия 5 ===
